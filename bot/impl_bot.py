...
def add_table(message):
    ...
    if space_after_url_ind == -1 or \
       space_after_url_ind == len(message.text) or \
       message.text[space_after_url_ind + 1] != '(' or message.text[-1] != ')':
        bot.send_message(message.chat.id,
                         "Please retry with syntax /add link\
                         (name copied from table)")
        return
    export_url = get_export_url(message.text[5:space_after_url_ind])
    if export_url[0] == 0:
        bot.send_message(message.chat.id,
                         "Please check your table link: " + export_url[1])
        return
...
def show_result(message):
    ...
    for request in search_requests:
        ...
        for i in range(len(table)):
            if table.iloc[i][0] == request[1] or \
               table.iloc[i][1] == request[1]:
                person_ind = i
                break
        if person_ind == -1:
            continue
        ...
        bot.send_message(message.chat.id, s_result)
